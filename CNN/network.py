...
def train(num_classes = 1, lr = 0.01, beta1 = 0.95, beta2 = 0.99, img_dim = 50, img_depth = 3, f = 5, num_filt1 = 8, num_filt2 = 8, batch_size = 32, num_epochs = 2, save_path = 'params.pkl'):
    ...
    # training data
    m = 957
    X = extract_data('dataset_50x50_MNIST/train-images-idx3-ubyte.gz', m, img_dim)
    y_dash = extract_labels('dataset_50x50_MNIST/train-labels-idx1-ubyte.gz', m).reshape(m,1)
    X-= int(np.mean(X))
    X/= int(np.std(X))
